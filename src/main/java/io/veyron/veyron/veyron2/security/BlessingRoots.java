...
public interface BlessingRoots {
	/**
	 * Marks <code>root</code> as an authoritative key for blessings that match
	 * <code>pattern</code>.
	 *
	 ...
	 * @throws VeyronException if there wan an error assigning the root.
	 */
	public void add(ECPublicKey root, BlessingPattern pattern) throws VeyronException;

	/**
	 * Returns <code>null</code> iff the provided root is recognized as an authority on a pattern
	 * that is matched by blessing.
	 *
	 ...
	 * lists out the contents of the roots.  Use <code>toString()</code> for a more succinct
	 * description.
	 *
