...
public interface Principal {
	 ...
	 *   <code>Bless(<other principal>, <google/alice, veyron/alice>, "friend", ...)</code>
	 *
	 * To discourage unconstrained delegation of authority, the interface requires at least one
	 * caveat to be provided. If unconstrained delegation is desired, the UnconstrainedUse function
	 * can be used to produce this argument.
	 *
	 * <code>with.publicKey</code> must be the same as the principal's public key.
	 *
	 * @param  key               PublicKey representing the Principal being blessed.
	...
	/**
	 * Marks the root principals of all blessing chains represented by 'blessings' as an authority
	 * on blessing chains beginning at that root.
	 *
	 * For example, if <code>blessings</code> represents the blessing chains ["alice/friend/spouse",
	 * "charlie/family/daughter"] then <code>addToRoots(blessing)</code> will mark the root public
	 ...
	 *
	 * @param  blessing        blessings to be used as authorities on blessing chains beginning at
	 *                         those roots.
	 * @throws VeyronException if there was an error assigning the said authorities.
	 */
	public void addToRoots(Blessings blessing) throws VeyronException;
}